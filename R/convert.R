...
convert <- function(input, outpath)
{
  ...
  ind_name <- lapply(chrom_raw, function(x)(names(x)))
  ...
  for(i in seq_along(chrom_raw)){
    chrom_raw[[i]] <- data.frame(chrom_raw[[i]], mz = rep(names(chrom_raw[[i]])[2]))
  }


  mz_raw <- lapply(chrom_raw, function(x)(clean_index(x$mz)))

  mz_split <- lapply(mz_raw, split_index)

  chrom_clean <- NULL
  for(i in seq_along(chrom_raw)){
    chrom_clean[[i]] <- data.frame(rt = chrom_raw[[i]]$time, mz = mz_split[[i]]$ms1, int = chrom_raw[[i]][,2],
  ...
  for(i in seq_along(chrom_split)){
    chrom_agg[[i]] <- aggregate(chrom_split[[i]]$int, by = list(chrom_split[[i]]$mzagg), sum)
  ...
  chrom_agg <- lapply(chrom_agg, function(x)(x[order(x$mz),]))


  hdnm <- header_names()

  hd_tmp <- data.frame(matrix(nrow = length(chrom_agg), ncol = length(hdnm)))
  names(hd_tmp) <- hdnm

  hd_tmp[,'seqNum'] <- hd_tmp[,'acquisitionNum'] <- seq(from = 1, to = nrow(hd_tmp), by = 1)
  ...
  tic <- unlist(lapply(chrom_agg, function(x)(sum(x$int))))
  bpi <- unlist(lapply(chrom_agg, function(x)(max(x$int))))
  bpmz <- unlist(lapply(chrom_agg, function(x)(x$mz[which(x$int == max(x$int))])))
  ...
  mzR::writeMSData(object = chrom_agg,file = destfile, header = hd_tmp,  outformat = 'mzml')
